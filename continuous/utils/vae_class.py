...
class VAE(nn.Module):
    ...
    def importance_sampling_estimator(self, state, action, beta, n_agents, num_samples=500):
        ...
        std_dec = torch.ones_like(mean_dec).to(self.device) * std_dec
        log_pxz = td.Normal(loc=mean_dec, scale=std_dec).log_prob(action)
        log_pxz = torch.chunk(log_pxz, n_agents, dim=-1)
        lll=[]
        for l_pxz in log_pxz:
            w = l_pxz.sum(-1) + log_pz.sum(-1) - log_qzx.sum(-1)
