...
class TD3MultiAgent(object):
    ...
    def __init__(
        ...
        self.policys = [MLPNetwork(num_in_pol, num_out_pol[i], hidden_dim=hidden_dim, constrain_out=True, discrete_action=discrete_action) for i in range(num_agent)]
        self.target_policys = [MLPNetwork(num_in_pol, num_out_pol[i], hidden_dim=hidden_dim, constrain_out=True, discrete_action=discrete_action) for i in range(num_agent)]
            
        self.critic = DoubleMLPNetwork(num_in_critic, num_out_critic, hidden_dim=hidden_dim, constrain_out=False)
        self.target_critic = DoubleMLPNetwork(num_in_critic, num_out_critic, hidden_dim=hidden_dim, constrain_out=False)
                
        for i in range(num_agent):
            hard_update(self.target_policys[i], self.policys[i])
        hard_update(self.target_critic, self.critic)
        
        self.policy_optimizers = [Adam(self.policys[i].parameters(), lr=lr) for i in range(num_agent)]
        self.critic_optimizer = Adam(self.critic.parameters(), lr=lr)
    ...
    def step(self, obs, explore=False):
        ...
        Take a step forward in environment for a minibatch of observations
        Inputs:
            obs (List[PyTorch Variable]): Observations for this agent
            explore (boolean): Whether or not to add exploration noise
        Outputs:
            action (List[PyTorch Variable]): Actions for all agents, 
        """
        actions = []
        for i in range(self.n_agent):
            action = self.policys[i](obs[i])
            if explore:
                x = action.clone().zero_()
                gaussian_noise = self.gaussian_noise_std * x.clone().normal_()
                action += Variable(gaussian_noise, requires_grad=False)
