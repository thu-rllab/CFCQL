...
class MATD3(object):
    ...
    def update(self, sample, agent_i, t, parallel=False):
        ...
        if self.omar:
            ...
            if self.is_mamujoco:
                ...
                for iter_idx in range(self.omar_iters):
                    ...
                    updated_mu = self.compute_softmax_acs(all_pred_qvals, cem_sampled_acs) #bs, ons, 1; bs, ons, 3
                    self.omar_mu = updated_mu
        ...
        if t % self.logging_interval == 0 and not self.no_log:
            dic = {}
            dic.update({"pol_loss_"+str(agent_i):pol_loss.item()})
            dic.update({"vf_loss_"+str(agent_i):vf_loss.item()})
            if self.cql:
                dic.update({"cql_term_"+str(agent_i):(self.cql_alpha * cql_term).item()})
            if self.omar:
                dic.update({"mimic_term_"+str(agent_i):(self.omar_coe * mimic_term).item()})
                dic.update({"pred_qvals_"+str(agent_i):((1 - self.omar_coe) * pred_qvals.mean()).item()})
