...
class Scenario(BaseScenario):
    def make_world(self, num_adversaries=1, num_good_agents=2, num_landmarks=2):
        world = World()
        
        # set any world properties first
        world.scenario_name = 'simple_crypto'
    ...
    def reset_world(self, world):
        ...
        goal_idx = -1
        for l_idx, l in enumerate(world.landmarks):
            if goal == world.landmarks[l_idx]:
                goal_idx = l_idx
                break
        self.goal_idx = goal_idx
        world.agents[1].color = goal.color

        tmp_key = np.random.choice(world.landmarks)
        tmp_key_idx = -1
        for l_idx, l in enumerate(world.landmarks):
            if tmp_key == world.landmarks[l_idx]:
                tmp_key_idx = l_idx
                break
        self.tmp_key_idx = tmp_key_idx
        world.agents[2].key = tmp_key.color
    ...
    def agent_reward(self, agent, world):
        ...
        for a in good_listeners:
            if (a.state.c.numpy() == np.zeros(world.dim_c)).all():
                continue
            else:
                good_rew -= np.sum(np.square(a.state.c.numpy() - agent.goal_a.color))
        for a in adversaries:
            if (a.state.c.numpy() == np.zeros(world.dim_c)).all():
                continue
            else:
                adv_l1 = np.sum(np.square(a.state.c.numpy() - agent.goal_a.color))
                adv_rew += adv_l1
    ...
    def adversary_reward(self, agent, world):
        ...
        rew = 0
        if not (agent.state.c.numpy() == np.zeros(world.dim_c)).all():
            rew -= np.sum(np.square(agent.state.c.numpy() - agent.goal_a.color))
        return rew
    ...
    def observation(self, agent, world):
        ...
        if agent.speaker:
            if prnt:
                print ('[speaker] state.c: {}, goal_color: {}, key: {}, confer: {}'.format(agent.state.c, goal_color, key, confer))
            return np.concatenate([goal_color] + [key])
        # listener
        if not agent.speaker and not agent.adversary:
            if prnt:
                print ('[listener] state.c: {}, key: {}, comm: {}, confer: {}'.format(agent.state.c, key, comm, confer))
            return np.concatenate([key] + comm)
        if not agent.speaker and agent.adversary:
            if prnt:
                print ('[adversary] state.c: {}, comm: {}, confer: {}'.format(agent.state.c, comm, confer))
